...
while(1):
    ...
    wtf, img = cv2.threshold(img, 132, 255, cv2.THRESH_BINARY)
    cv2.imshow('erzhi', img)
    # ############## img灰度化加二值化 # input:img     output:img
    cany = cv2.GaussianBlur(img, (3, 3), 0)
    ...
    try:
        ...
        pass
    try:
        print('横线 angleheng '+str(len(angleheng))+str(sum(angleheng)/len(angleheng))+'竖线 angleshu'+str(len(angleshu))+str(sum(angleshu)/len(angleshu)))
        angleshu, angleheng = [], []
    except:
        print('线炸了')
