...
for teststep in step:
    ...
    while(1):
        ...
        # ret3, img3 = cap3.read()
        undistorted_img = img
        if img[50-kuandu:50+kuandu,0:90].sum()/255 + img[0:100,45-kuandu:45+kuandu].sum()/255 - (2*kuandu*90 - 2*kuandu*100)*baifenbi >= 0 and time.time()-stickin>=0.2:
            shiziflag = 1
        else:
            shiziflag = 0
        ...
        if shuzhe[0] == 0 and shuzhe[1] == 0 and shuzhe[2] == 1 and shifoupandingflag == 1 and time.time()-stickin >= 0.2:
            get_shu_pass = get_shu_pass + 1
            get_shu = get_shu_pass
        if img[0:100, 45-kuandu:45+kuandu].sum()/255 - (2*kuandu*100)*baifenbi >= 0 and shifoupandingflag == 2 and time.time()-stickin >= 0.2:
            get_shu = get_shu+1
        ...
        hengzhesao1 = np.where(hengzhesao>=88, 1, 0)
        ...
        if hengzhe[0] == 0 and hengzhe[1] == 0 and hengzhe[2] == 1 and shifoupandingflag == 1 and time.time()-stickin >= 0.2:
            get_heng_pass = get_heng_pass + 1
            get_heng = get_heng_pass
        if img[50-kuandu:50+kuandu,0:90].sum()/255 - (2*kuandu*90)*baifenbi >= 0 and shifoupandingflag == 2 and time.time()-stickin>=0.2:
            get_heng = get_heng+1
        ...
        if get_heng < teststep[0]-1 and get_shu < teststep[1]-1:
            jiansuflag = 0
            shifoupandingflag = 1
            ser.write(str('x') + '0' + '000.0' + 't' + '0' + '00.00' + str(0) + str(0) + str(jiansuflag) + '1314' + '\0')
