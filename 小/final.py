...
# -------------------------------变量初始化main的-----------------------------------------
hengzhe,shuzhe = [1, 1, 1], [1, 1, 1]
hengzheROL,shuzheROL = [1, 1, 1], [1, 1, 1]
hengzheROR,shuzheROR = [1, 1, 1], [1, 1, 1]
get_heng = 0
get_shu = 0
        ...
        if for_step[0] == 0 or for_step[1] == 0:
                ...
                try:
                    ...
                    if DIR == 1 or DIR == 2:
                        if get_heng_pass < nowstep:
                            ...
                            hengzhesao1 = np.where(hengzhesao >= 76, 1, 0)
                        ...
                        if get_heng_pass == nowstep:
                            ...
                            for i in range(hengzhesao1.shape[0] - 8 + 1):
                                believehengzhe = np.append(believehengzhe, hengzhesao1[i:i + 8].sum())
                            hengxianloc = np.average(np.where(believehengzhe >= 4))
                            if hengxianloc <= 65 and hengxianloc >= 25:
                                get_shu_pass ,get_heng_pass = 0, 0
                                delta = []
                                delta1 = []
                                delta2 = []
            ...
            while (1):
                try:
                    ret, img = cap.read()
                    result = img.copy()
                    (lower, upper) = ([0, 0, 136], [255, 255, 255])
                    lower = np.array(lower, dtype="uint8")
                    upper = np.array(upper, dtype="uint8")
                    mask = cv2.inRange(img, lower, upper)
                    img = cv2.bitwise_and(img, img, mask=mask)
